...
{
  "originalServer": {
    "url": "https://example.com/playlist.m3u",  // 兼容字段
    "urls": [                                   // 多订阅源配置
      {
...
**多订阅源说明：**
- 系统会自动从所有启用的订阅源加载频道并合并
- 每个订阅源可以独立命名和启用/禁用
- 旧的单订阅源配置会自动迁移到新格式
- 使用 `/admin sources` 命令管理多个订阅源
...
#### 配置管理
- `/changem3u <新链接>` - 修改M3U订阅链接（兼容旧方式）
- 推荐使用 `/admin sources` 命令管理多个订阅源
